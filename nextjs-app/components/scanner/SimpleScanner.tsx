...
const SimpleScanner: React.FC<SimpleScannerProps> = ({
  ...
  const startScanner = async () => {
    try {
      ...
      const backCamera = videoDevices.find((device: any) => /back|rear|environment/i.test(device.label));
        ...
        await codeReaderRef.current.decodeFromVideoDevice(
          ...
          (result: any, decodeError: any) => {
            ...
            if (decodeError && !(decodeError instanceof NotFoundException)) {
              console.warn("[SimpleScanner] Decode error:", decodeError);
            }
          }
        );
      }
          ...
          <div className="relative bg-slate-900 rounded-2xl overflow-hidden aspect-video mb-6">
            ...
            {scanning && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-64 h-64 border-4 border-blue-500 rounded-3xl animate-pulse" />
              </div>
            )}
          </div>

          <div className="text-center">
            <p className="text-sm text-slate-600 font-medium">Arahkan kamera ke QR code atau barcode</p>
            <p className="text-sm text-slate-400 mt-2">Scanner akan otomatis menutup setelah berhasil</p>
          </div>
        </div>
